...
BOOL CALLBACK DlgFunc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam){
	...
	case WM_INITDIALOG: {
							...
							while (1) {
								...
								if (!nRes || ress[nRes - 1].w != d.dmPelsWidth || ress[nRes - 1].h != d.dmPelsHeight) {
									...
									nRes++;
									_snprintf(s, 500, "%d x %d", d.dmPelsWidth, d.dmPelsHeight);
							...
							if (setupcfg->nVsync) {
								SendDlgItemMessage(hWnd, IDC_ENABLE_VSYNC, BM_SETCHECK, 1, 1);
							}
							/*if (setupcfg->nVsync)
	...
	case WM_COMMAND:
		...
		case IDOK: {
					   ...
					   setupcfg->nVsync = SendDlgItemMessage(hWnd, IDC_ENABLE_VSYNC, BM_GETCHECK, 0, 0);
...
int OpenSetupDialog(fwzSettings * s) {
  setupcfg = s;
#ifdef DEBUG
   int r = DialogBox(s->hInstance, MAKEINTRESOURCE(IDD_SETUP), GetForegroundWindow(), Debug_DlgProc);
   return r;
#else
  return DialogBox(s->hInstance, MAKEINTRESOURCE(IDD_SETUP), GetForegroundWindow(), DlgFunc);
#endif
