...
float noise( in vec3 x )
{
    ...
    float a = texture2D( iChannel0, x.xy/256.0 + (p.z+0.0)*5.7123 ).x;
    float b = texture2D( iChannel0, x.xy/256.0 + (p.z+1.0)*5.7123 ).x;
	return mix( a, b, f.z );
}*/
...
void main( void )
{
	...
	ns = noise(vec3(p, iGlobalTime));
	gl_FragData[0]= vec4(ns, 0.0, 0.0, 1.0);
	gl_FragData[1]= vec4(0.0, ns, 0.0, 1.0);
	gl_FragData[2]= vec4(ns, ns, ns, 1.0);
	gl_FragDepth = ns;
	return;
#endif
	...
	// camera	
	float off = 0.0;//step( 0.001, iMouse.z )*6.0*iMouse.x/iResolution.x;
	float time = 0.0;//2.7+iGlobalTime + off;

	vec3 ro = cameraPosition;	
	
	//vec3 rd =  normalize(viewDirection);
	//vec3 ta = path( time+1.6 );
	//vec3 ro = vec3( cameraPosition * cameraLookDirection ) ;
	//ro.x -= 1.0;

//	vec3 ta = vec3( 0.0, 0.0, 1.0 ) + ro;

//	float tx = 2.7+iGlobalTime + off;
	...
	/*
	// camera tx
	vec3 cw = normalize(ta-ro); // ray direction
	//vec3 cp = vec3(sin(roll), cos(roll),0.0); // up vector
	...
	**/
	...
	if( t>0.0 )
	{
		...
		vec3 lin  = sun*vec3(1.64,1.27,0.99)*pow(vec3(sha),vec3(1.0,1.2,1.5))*(0.8+0.2*occ);
				lin += sky*vec3(0.16,0.20,0.28)*occ; // sky color
				lin += bac*vec3(0.40,0.28,0.20)*occ; // background (fog) color
				lin += amb*vec3(0.15,0.15,0.25)*occ; // ambient color
				lin += lav*vec3(0.0, 0.0, 0.0); // lava color


		// surface shading/material		
		col = texcube( iChannel2, 0.5*pos, nor ).xyz;
		col = col*(0.2+0.8*texcube( iChannel2, 4.0*vec3(2.0,8.0,2.0)*pos, nor ).x);
		vec3 grass = vec3(1.0,0.9,0.2);
		grass *= texture2D( iChannel2, pos.xz ).xyz;
		col = mix( col, 0.8*grass, hh );
		
		float vv = smoothstep( 0.0, 0.8, nor.y )*smoothstep(0.0, 0.1, pos.y-0.8 );
		grass = vec3(0.2,0.45,0.1);
		grass *= texture2D( iChannel2, 30.0*pos.xz ).xyz;
		grass += 0.2*texture2D( iChannel2, 1.0*pos.xz ).xyz;
		vv *= smoothstep( 0.0, 0.5, texture2D( iChannel2, 0.1*pos.xz + 0.01*nor.x ).x );
		col = mix( col, grass*1.1, vv );
		...
		float zDepth = dot(pos - cameraPosition, cameraLookDirection);
...
//	col = mix( col, res.xyz, res.w );
	...
	float zc = ( mvm * vec4( pos-cameraPosition, 1.0 ) ).z;
	float wc = ( mvm * vec4( pos-cameraPosition, 1.0 ) ).w;
		
	gl_FragData[0] = vec4( col, 1.0 );
	//gl_FragData[1] = vec4( cu, 1.0 );
	//gl_FragData[2] = vec4( cv, 1.0 ); 

	/*
	gl_FragColor.r = pos;
	gl_FragColor.g = cameraPosition;
	gl_FragColor.b = 0.0;
	*/
	//if (p.x > 0.0)
	//	gl_FragColor.rgb = fdepth;  
		//gl_FragColor.rgb = zc/wc;
		
	
	

	//float v = noise(vec3(gl_FragCoord.x, gl_FragCoord.y, 0.0));

	//gl_FragColor = vec4( v, v, v, 1.0 );

			//depth
	/*
	//vec3 intersectionPoint = distanceTravelled;	
	float zDepth = dot(position-cameraPosition, cameraLookDirection);
