...
const char* const voxRenderAdd_frag =
 ...
 "{"
   "if(Color==vec4(0))"
     "discard;"
   "gl_FragData[0]=Color*pow(1.-min(length(CornerVec),1.),4.);"
 "}";

const char* const voxRender_vert =
 "#version 150 compatibility\n"
 "#extension GL_EXT_gpu_shader4:enable\n"
 "uniform mat4 matProj,matView;"
 "uniform sampler2D PosTex,ColorTex;"
 "uniform sampler3D NoiseTex;"
 "uniform float Time,CamShake;"
 "varying vec2 CornerVec;"
 "varying vec4 Color;"
 "uniform float Brightness;"
 "void main()"
 "{"
   "CornerVec=gl_MultiTexCoord0.xy*2.-1.;"
   "ivec2 puv=ivec2(gl_Vertex.xy);"
   "vec4 posLife=texelFetch(PosTex,puv,0);"
   "float noiseTime=Time*.02;"
   "vec3 pos=posLife.xyz+(vec3(texture3D(NoiseTex,vec3(noiseTime,0,0)).x,texture3D(NoiseTex,vec3(0,noiseTime,0)).x,texture3D(NoiseTex,vec3(0,0,noiseTime)).x)*2.-1.)*CamShake;"
...
const char* const voxRender_frag =
 "varying vec2 CornerVec;"
 "varying vec4 Color;"
 "void main()"
 "{"
   "if(Color==vec4(0))"
     "discard;"
   "gl_FragData[0]=vec4(Color.xyz,Color.w*pow(1.-min(length(CornerVec),1.),4.));"
 "}";

const char* const voxelsVel_vert =
 "void main()"
 "{"
   "gl_Position=gl_Vertex;"
 "}";

const char* const voxelsVel_frag =
 "#version 150 compatibility\n"
 "#extension GL_EXT_gpu_shader4:enable\n"
 "uniform sampler2D texOldPos,texOldVel;"
 "uniform sampler3D texNoise,texNoise2;"
 "uniform float time,timeStep,accSpeed,damping;"
 "float f(vec3 x)"
 "{"
 ...
 "{"
   "vec3 y=fract(x);"
   "return normalize(vec3(f(y+vec3(1./256.,0,0))-f(y-vec3(1./256.,0,0)),f(y+vec3(0,1./256.,0))-f(y-vec3(0,1./256.,0)),f(y+vec3(0,0,1./256.))-f(y-vec3(0,0,1./256.))));"
 "}"
 "void main()"
 "{"
   "ivec2 puv=ivec2(gl_FragCoord.xy);"
   "gl_FragData[0]=vec4(texelFetch(texOldVel,puv,0).xyz*damping+g(texelFetch(texOldPos,puv,0).xyz*.012)*(timeStep+.0001)*accSpeed,1);"
 "}";

const char* const voxelsPos_vert =
 "void main()"
 "{"
   "gl_Position=gl_Vertex;"
 "}";

const char* const voxelsPos_frag =
 "#version 150 compatibility\n"
 "#extension GL_EXT_gpu_shader4:enable\n"
 "uniform sampler2D texOldPos,texVel,posTex;"
 "uniform float time,timeStep,damping,velSpeed;"
 "void main()"
 "{"
   "ivec2 puv=ivec2(gl_FragCoord.xy);"
...
const char* const voxels_vert =
 "#version 330\n"
 "uniform mat4 matView,matModelView,matProjection;"
 "uniform mat3 matNormal;"
 "uniform sampler2D texPos,texCol;"
 ...
 "void main()"
 "{"
   "float CamShake=1.,Brightness=1.,noiseTime=time*.02;"
   "ivec2 puv=ivec2(pos.xy);"
   "vec4 posLife=texelFetch(texPos,puv,0);"
   "vec3 pos=posLife.xyz+(vec3(texture(texNoise,vec3(noiseTime,0,0)).x,texture(texNoise,vec3(0,noiseTime,0)).x,texture(texNoise,vec3(0,0,noiseTime)).x)*2.-1.)*CamShake;"
...
const char* const voxels_geom =
 ...
 "float snoise(vec4 v)"
 "{"
   "const vec4 C=vec4(.138197,.276393,.41459,-.447214);"
   "\n#define F4 0.309016994374947451\n"
   "vec4 i=floor(v+dot(v,vec4(F4))),x0=v-i+dot(i,C.xxxx),i0;"
   "vec3 isX=step(x0.yzw,x0.xxx),isYZ=step(x0.zww,x0.yyz);"
   ...
   "i0.z+=isYZ.z;"
   "i0.w+=1.-isYZ.z;"
   "vec4 i3=clamp(i0,0.,1.),i2=clamp(i0-1.,0.,1.),i1=clamp(i0-2.,0.,1.),x1=x0-i1+C.xxxx,x2=x0-i2+C.yyyy,x3=x0-i3+C.zzzz,x4=x0+C.wwww;"
   "i=mod(i,289.);"
   "float j0=permute(permute(permute(permute(i.w)+i.z)+i.y)+i.x);"
   "vec4 j1=permute(permute(permute(permute(i.w+vec4(i1.w,i2.w,i3.w,1.))+i.z+vec4(i1.z,i2.z,i3.z,1.))+i.y+vec4(i1.y,i2.y,i3.y,1.))+i.x+vec4(i1.x,i2.x,i3.x,1.)),ip=vec4(1./294.,1./49.,1./7.,0.),p0=grad4(j0,ip),p1=grad4(j1.x,ip),p2=grad4(j1.y,ip),p3=grad4(j1.z,ip),p4=grad4(j1.w,ip),norm=taylorInvSqrt(vec4(dot(p0,p0),dot(p1,p1),dot(p2,p2),dot(p3,p3)));"
   "p0*=norm.x;"
   "p1*=norm.y;"
   "p2*=norm.z;"
   "p3*=norm.w;"
 ...
 "{"
   "vec3 v=vec3(0.);"
   "float f=freq,g=gain;"
   "for(float oct=1.;oct<=numOcts;oct+=1.)"
     "v+=g*vec3(snoise(vec4(p*f+vec3(4.,1.2,.5+oct),time)),snoise(vec4(p*f+vec3(6.1,4.8+oct,7.3),time)),snoise(vec4(p*f+vec3(3.7+oct,7.4,3.7),time))),f*=lacunarity,g*=roughness;"
   "return v;"
 "}"
 "uniform mat4 projectionMatrix;"
 "uniform sampler2D input0;"
 "uniform int iGridSize;uniform mat4 viewMatrix;uniform mat4 modelViewMatrix;uniform float fCurveZ;uniform float fRadialBlockDisplaceAmount;layout(points)in;layout(triangle_strip,max_vertices=24)out;uniform float fSeed1;uniform float fSeed2;uniform float fExtrude;uniform float fTime;uniform float fPrecalcPercentage;uniform int iIsPrecalc;uniform float fRadius;uniform int iScene;"
 "float rand(vec2 co)"
 "{"
 ...
 "{"
   "viewspaceNormal=normalize(normal.xyz),viewspacePosition4=vertices[i],viewspacePosition3=vertices[i].xyz,gl_Position=projectionMatrix*vertices[i],EmitVertex(),viewspacePosition4=vertices[j],viewspacePosition3=vertices[j].xyz,gl_Position=projectionMatrix*vertices[j],EmitVertex(),viewspacePosition4=vertices[k],viewspacePosition3=vertices[k].xyz,gl_Position=projectionMatrix*vertices[k],EmitVertex(),viewspacePosition4=vertices[l],viewspacePosition3=vertices[l].xyz,gl_Position=projectionMatrix*vertices[l],EmitVertex(),EndPrimitive();"
 "}"
 ...
 "{"
   ...
   "emitface(0,1,3,2,vm*vec4(0,0,1,0));"
   "emitface(5,4,6,7,vm*vec4(0,0,-1,0));"
   "emitface(4,0,7,3,vm*vec4(1,0,0,0));"
   "emitface(1,5,2,6,vm*vec4(-1,0,0,0));"
   "emitface(1,0,5,4,vm*vec4(1,0,0,0));"
   "emitface(3,2,7,6,vm*vec4(-1,0,0,0));"
 "}"
 "void main()"
 "{"
   "vec4 p=gl_in[0].gl_Position;"
   "vec2 pi=floor(p.xy*iGridSize);"
   "float resu=160,resv=240,u=pi.x,v=pi.y,i=resu*v+u,resi=resu*resv;"
   "mat4 t=mtranslate(p.x,p.y,0.);"
   "emitcube(t);"
 "}";

const char* const voxels_frag =
 "#version 330\n"
 "in vec2 cornerVec;"
 "in vec4 col;"
 "in vec3 normal,vert;\n"
 ...
 "vec4 diffuse=vec4(.024,.02,.01,1.),ambient=vec4(.3,.1,.1,1.),specular=vec4(.3,.2,.1,1.);"
 "float shininess=.5;"
 "void main()"
 "{"
...
const char* const terrainTest_frag =
 ...
 "{"
   ...
   "mat3 rot=rotz(pi2*.5);"
   "rot*=rotx(pi2*.75);"
   "float distort=fbm(pos.xxz+offset)*.2,dist=sdSphere(pos-vec3(0.,20.,0.)+distort+offset,vec4(20.)),head=sdSphere(pos-vec3(10.,55.,10.)+distort+offset,vec4(10.));"
   "dist=smin(dist,head,2.);"
   "float hat=sdTorus82((pos-vec3(20.,75.,20.)+distort+offset)*rotx(0.),vec2(4.,4.));"
   "hat=smin(hat,sdTorus82((pos-vec3(20.,73.,20.)+distort+offset)*rotx(0.),vec2(10.,1.)),2.);"
   "if(hat<dist)"
     "materialId=MAT_HAT;"
   "dist=smin(dist,hat,2.);"
   "float nose=sdSphere(pos-vec3(18.,65.,28.)+distort+offset,2.);"
   "if(nose<dist)"
     "materialId=MAT_NOSE;"
   "dist=smin(dist,nose,2.);"
   "float eyes=sdSphere(pos-vec3(15.,68.,27.)+distort+offset,1.);"
   "eyes=min(eyes,sdSphere(pos-vec3(22.,68.,27.)+distort+offset,1.));"
   "if(eyes<dist)"
     "materialId=MAT_EYE;"
   "dist=smin(dist,eyes,.2);"
   "float buttons=sdSphere(pos-vec3(18.,45.,36.5)+distort+offset,1.5);"
   "buttons=min(sdSphere(pos-vec3(18.4,40.,37.)+distort+offset,1.5),buttons);"
   "buttons=min(sdSphere(pos-vec3(18.,35.,37.5)+distort+offset,1.5),buttons);"
   "buttons=min(sdSphere(pos-vec3(19.1,50.,33.5)+distort+offset,1.5),buttons);"
   "buttons=min(sdSphere(pos-vec3(18.7,54.,31.)+distort+offset,1.5),buttons);"
   "if(buttons<dist)"
     "materialId=MAT_EYE;"
   "dist=smin(dist,buttons,.5);"
   "float mouth=10000.;"
   "mouth=min(sdSphere(pos-vec3(14.,63.,27.5)+distort+offset,.5),mouth);"
   "mouth=min(sdSphere(pos-vec3(16.,62.,28.5)+distort+offset,.5),mouth);"
   "mouth=min(sdSphere(pos-vec3(18.,61.,28.5)+distort+offset,.5),mouth);"
   "mouth=min(sdSphere(pos-vec3(20.,61.,28.5)+distort+offset,.5),mouth);"
   "mouth=min(sdSphere(pos-vec3(22.,62.,28.5)+distort+offset,.5),mouth);"
   "mouth=min(sdSphere(pos-vec3(24.,63.,28.)+distort+offset,.5),mouth);"
   "if(mouth<dist)"
     "materialId=MAT_EYE;"
 ...
 "{"
   "float res=1.,t=0.,h=1.;"
   "for(int i=0;i<60;i++)"
     "h=map(ro+rd*t,masked).x,res=min(res,k*h/t),t+=clamp(h,.02,1.);"
...
const char* const sceneRender_vert =
 "uniform mat4 matView,matModelView,matProjection;"
 "uniform mat3 matNormal;"
 "varying vec3 norm,worldPos;"
 "void main()"
 "{"
   "norm=(matModelView*vec4(gl_Normal,0)).xyz,worldPos=(matModelView*gl_Vertex).xyz,gl_Position=matProjection*matModelView*gl_Vertex;"
 "}";

const char* const sceneRender_frag =
 "varying vec3 norm,worldPos;"
 "uniform vec3 Color,LightDir;"
 "void main()"
 "{"
   "gl_FragData[0]=vec4(worldPos,1.f),gl_FragData[1]=vec4(Color*vec3(dot(normalize(norm),normalize(LightDir))*.5+.5),1),gl_FragData[2]=vec4(norm,1.);"
 "}";
...
const char* const particles_geom =
 ...
 "{"
   ...
   "o*=size;"
   "if(size>.1)"
     "quadSize=o.x*50.,gl_Position=projectionMatrix*(p+o.xzyy),quadCoord=vec2(1.,-1.),EmitVertex(),gl_Position=projectionMatrix*(p+o.xxyy),quadCoord=vec2(1.,1.),EmitVertex(),gl_Position=projectionMatrix*(p+o.zzyy),quadCoord=vec2(-1.,-1.),EmitVertex(),gl_Position=projectionMatrix*(p+o.zxyy),quadCoord=vec2(-1.,1.),EmitVertex();"
 "}";
...
const char* const Noise3DLinear_frag =
 "uniform sampler3D t;"
 "uniform int a;"
 "float s(vec3 x)"
 "{"
 ...
 "{"
   "float x=float(a);"
   "gl_FragData[0]=vec4(1)*(s(vec3(gl_FragCoord.xy,x))*.5+s(vec3(gl_FragCoord.xy,x)*2.)*.25+s(vec3(gl_FragCoord.xy,x)*4.)*.125+s(vec3(gl_FragCoord.xy,x)*8.)*.0625+s(vec3(gl_FragCoord.xy,x)*16.)*.03125);"
 "}";

const char* const noise_vert =
 "void main()"
 "{"
   "gl_Position=gl_Vertex;"
 "}";

const char* const noise_frag =
 "uniform sampler3D t;"
 "uniform float time;"
 "void main()"
 "{"
   "vec2 p=gl_FragCoord.xy/vec2(800.,600.);"
   "vec3 pos=vec3(p.xy,time*.01);"
   "float nz=texture3D(t,pos),n1=0;"
   "vec4 col=vec4(nz,nz,nz,1.);"
   "gl_FragColor=col;"
 "}";

const char* const initVox_vert =
 "void main()"
 "{"
   "gl_Position=gl_Vertex;"
 "}";

const char* const initVox_frag =
 "#version 150 compatibility\n"
 "#extension GL_EXT_gpu_shader4:enable\n"
 "uniform sampler3D NoiseTex;"
 "float f(vec3 x)"
 ...
 "vec3 g(vec3 x)"
 "{"
   "return vec3(f(x+vec3(1./256.,0,0))-f(x-vec3(1./256.,0,0)),f(x+vec3(0,1./256.,0))-f(x-vec3(0,1./256.,0)),f(x+vec3(0,0,1./256.))-f(x-vec3(0,0,1./256.)));"
 "}"
 "void main()"
 "{"
   "vec3 x=g(vec3(gl_FragCoord.xy/vec2(800,600),0))*20.f;"
   "gl_FragData[0]=vec4(x,fract(x.x*100.));"
 "}";
