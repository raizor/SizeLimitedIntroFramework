...
float terrain( vec2 x )
{
    ...
    height -= 0.04*(noise( (5.0+0.1*time)*z ))+(0.1*time);

	height -= sin(z).x+(0.1*time);
	height += sin(z).y+(0.1*time);
...
vec2 distToSnowman(in vec3 pos, in vec3 offset) // returns dist, materal-id
{
	...
	// body
	float dist = sdSphere(pos-vec3(0.0, 20.0, 0.0)+vec3(distort)+vec3(offset), vec4(20.0));
	//dist = min(sdTorus82(pos-vec3(18.4, 40.0, 37.0)+distort+offset, vec2(5.0)), dist);

	// head
	float head = sdSphere(pos-vec3(10.0, 55.0, 10.0)+vec3(distort)+vec3(offset), vec4(10.0));
	dist = smin(dist, head, 2.0);
						
    // hat			
	float hat = sdTorus82((pos-vec3(20.0, 75.0, 20.0)+distort+vec3(offset))*rotx(0.0), vec2(4.0, 4.0)); // top 
	...
	// nose
	float nose = sdSphere(pos-vec3(18.0, 65.0, 28.0)+vec3(distort)+vec3(offset), vec4(2.0));
	if (nose<dist)
		materialId = MAT_NOSE;
	dist = smin(dist, nose, 2.0);

	// eyes
	float eyes = sdSphere(pos-vec3(15.0, 68.0, 27.0)+vec3(distort)+vec3(offset), vec4(1.0));	
	...
	*/
	...
	float buttons = sdSphere(pos-vec3(18.0, 45.0, 36.5)+distort+offset, vec4(1.5));
	
	buttons = min(sdSphere(pos-vec3(18.4, 40.0, 37.0)+distort+offset, vec4(1.5)), buttons);
	buttons = min(sdSphere(pos-vec3(18.0, 35.0, 37.5)+distort+offset, vec4(1.5)), buttons);
	buttons = min(sdSphere(pos-vec3(19.1, 50.0, 33.5)+distort+offset, vec4(1.5)), buttons);
	buttons = min(sdSphere(pos-vec3(18.7, 54.0, 31.0)+distort+offset, vec4(1.5)), buttons);
	...
	// mouth
	float mouth = 10000.0;
	mouth = min(sdSphere(pos-vec3(14.0, 63.0, 27.5)+distort+offset, vec4(0.5)), mouth);
	mouth = min(sdSphere(pos-vec3(16.0, 62.0, 28.5)+distort+offset, vec4(0.5)), mouth);
	mouth = min(sdSphere(pos-vec3(18.0, 61.0, 28.5)+distort+offset, vec4(0.5)), mouth);

	mouth = min(sdSphere(pos-vec3(20.0, 61.0, 28.5)+distort+offset, vec4(0.5)), mouth);
	mouth = min(sdSphere(pos-vec3(22.0, 62.0, 28.5)+distort+offset, vec4(0.5)), mouth);
	mouth = min(sdSphere(pos-vec3(24.0, 63.0, 28.0)+distort+offset, vec4(0.5)), mouth);
