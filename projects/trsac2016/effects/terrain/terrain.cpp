...
void terrain::draw(float itime)
{
	...
	SYNC_OBJ_MIX_1 = (float)sync_get_val(objectMix, ROCKET_ROW);

	//sunPos
	sunPos[0] = (float)sync_get_val(sunPosX, ROCKET_ROW);
	sunPos[1] = (float)sync_get_val(sunPosY, ROCKET_ROW);
	sunPos[2] = (float)sync_get_val(sunPosZ, ROCKET_ROW);
...
//	glUniformMatrix4fv(iLoc, 1, false, matrix);	
	...
	glUniform1f(iLoc, (float)sync_get_val(textAmount, ROCKET_ROW));

	iLoc = glGetUniformLocation(txt3d->shaderText->shaderProg, "time");	
	glUniform1f(iLoc, itime);

	int txtNum = (int)sync_get_val(textNum, ROCKET_ROW);
	if (txtNum > -1)
	{
