...
float EPSILON_NRM	= 0.1 / g_resolution.x;
...
//const vec3 COL_SEA_BASE = vec3(0.1,0.19,0.22);

const vec3 COL_SEA_BASE = vec3(0.05,0.1,0.1);
const vec3 SEA_WATER_COLOR = vec3(0.1,0.6,0.8);
float SEA_TIME = time * SEA_SPEED;
mat2 octave_m = mat2(1.6,1.2,-1.2,1.6);

// materials
const float MAT_SEA = 1.0;
const float MAT_OBJ = 2.0;
...
vec3 background(vec3 p, vec3 d)//render background
{
	...
	return 2.5*color*(noise(d)+.3*pow(noise(d*4.0),2.0));
...
vec3 getSeaColor(vec3 p, vec3 n, vec3 l, vec3 eye, vec3 dist, vec3 bg) {  
	...
	//vec3 reflected = background(light,reflect(eye,n));   
    vec3 refracted = COL_SEA_BASE + diffuse(n,l,80.0) * SEA_WATER_COLOR * 0.12; 
...
vec2 march(vec3 ori, vec3 dir, out vec3 p) {  
	...
	float tm = 0.0;
    float tx = 1000.0;    
    float hx = map_water(ori + dir * tx);
    if(hx > 0.0) return vec2(tx, 0);   
    float hm = map_water(ori + dir * tm);    
    float tmid = 0.0;
...
void main() {
	...
	*/
    ...
    vec3 dist = p - ori;
    vec3 n = getNormal(p, dot(dist,dist) * EPSILON_NRM, obj.y);
    vec3 light = normalize(vec3(0.0,1.0,0.8)); 
    ...
    vec3 color = mix(
      ...
      */  
	  ...
	  vec3 bg = background(light,dir);

	  vec3 color = mix(
        //background(light,dir),
		getSkyColor(dir),
        getSeaColor(p,n,light,dir,dist, bg),
    	pow(smoothstep(0.0,-0.05,dir.y),0.3));
    // post
