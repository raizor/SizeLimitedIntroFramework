#include "intro_fnuque.h"

effect_fnuque::effect_fnuque(UINT32 texNoise, UINT32 texColor, UINT32 texNoise3d, UINT32 texNoise3dFix)
{
	...
	font = new fontBitmap();
	text = new fontBitmapText(font, "abcdefghi");

#ifdef DEBUG
	shaderMain = new shader(
		"shaders\\include\\base.vert",
		NULL,
		"shaders\\include\\base.frag",
		"",
		"water"
		);
#else
...
void effect_fnuque::draw_effect(float itime)
{

	text->getCullingBuffer();
	...
	glBindTexture(GL_TEXTURE_2D, global::GetState()->fboCulling->colorBufferID[0]);
...
void effect_fnuque::draw(float itime)
{
	glEnable(GL_DEPTH_TEST);
