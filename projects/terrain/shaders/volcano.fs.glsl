...
void main( void )
{
    ...
    if( t>0.0 )
	{
		...
		float sha = 0.0; if( dif>0.01) sha=softshadow(pos,lig,0.01,32.0);
		...
		vec3 lin  = 1.3*dif*vec3(0.9,0.70,0.55)*sha*(0.8+0.2*occ);
		     lin += 0.8*bac*vec3(0.5,0.35,0.25)*occ;
		     lin += 0.8*sky*vec3(0.2,0.25,0.35)*occ;
		     lin += 0.6*amb*vec3(0.3,0.25,0.25)*occ;
		     lin += 1.5*lav*vec3(2.0,0.41,0.00);
