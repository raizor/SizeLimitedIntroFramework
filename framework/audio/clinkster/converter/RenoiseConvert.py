		...
		if tr in [int(xmt) for xmt in xseq.MutedTracks.MutedTrack]:
			...
			for xline in xtrack.Lines.Line:
				...
				if index < nlines:
					...
					if len(xcol.Note) > 0 and str(xcol.Note) != "---":
						note = Note(line, posn, patn, xcol.Note, xcol.Instrument, xcol.Volume)
						if note.velocity == 0:
							raise InputException("Track '%s' pattern %d position %d: Velocity 0 used" % (tname, patn, index))
						notes.append(note)
				else:
					outside_pattern += 1
		pattern_top += nlines
...
def pickupDelay(xdevices, delay_lengths, delay_strength, tname, ticklength):
	if len(xdevices.DelayDevice) and str(xdevices.DelayDevice.IsActive) == "true":
...
def makeTracks(version, xsong, ticklength):
	...
	for ii,xinst in enumerate(xsong.Instruments.Instrument):
		params = [float(v) for v in xinst.PluginProperties.PluginDevice.Parameters.Parameter.Value]
		if params:
			instrument = Instrument(ii, str(xinst.Name), params)
			instrument.volume = makeVolume(xinst.PluginProperties.Volume)
	...
	for tr,xtrack in enumerate(xsong.Tracks.SequencerTrack):
		...
		while len(xdevices.SendDevice) and str(xdevices.SendDevice.IsActive) == "true":
			if len(xdevices.DelayDevice) and str(xdevices.DelayDevice.IsActive) == "true":
...
def makeMusic(xsong):
	vstnames = set(str(v) for v in xsong.Instruments.Instrument.PluginProperties.PluginDevice.PluginIdentifier)
	if len(vstnames) > 1:
	...
	if len(xgsd.PlaybackEngineVersion) and int(xgsd.PlaybackEngineVersion) >= 4:
