...
const struct sync_track *sync_get_track(struct sync_device * d, const char *trackName)
{
	struct sync_track *t;
	int idx = sync_find_track(&d->data, trackName);
	if (idx >= 0)
		return d->data.tracks[idx];

	idx = sync_create_track(&d->data, trackName);
	t = d->data.tracks[idx];
